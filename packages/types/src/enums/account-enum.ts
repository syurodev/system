...
export const SUBSCRIPTION_FEATURES = {
  [SubscriptionPlanType.FREE]: {
    label: SubscriptionPlanTypeLabels[SubscriptionPlanType.FREE],
    price: 0,
    features: [
      'Đọc/xem nội dung công khai',
      'Xem quảng cáo',
      'Anime chất lượng SD',
    ...
    limitations: [
      'Có quảng cáo',
      'Không truy cập nội dung premium',
      'Chất lượng video giới hạn',
    ],
  },
  [SubscriptionPlanType.PREMIUM]: {
    label: SubscriptionPlanTypeLabels[SubscriptionPlanType.PREMIUM],
    price: 99000, // VND per month
    features: [
      'Tất cả tính năng Free',
      'Loại bỏ quảng cáo',
      'Truy cập nội dung premium',
      'Anime chất lượng HD/FHD',
      'Đọc trước (Early Access)',
    ],
    limitations: [
      'Không có quyền lợi VIP đặc biệt',
    ],
  },
  [SubscriptionPlanType.VIP]: {
    label: SubscriptionPlanTypeLabels[SubscriptionPlanType.VIP],
    price: 199000, // VND per month
    features: [
      'Tất cả tính năng Premium',
      'Truy cập nội dung độc quyền',
      'Giảm giá có đánh trên sàn TMĐT',
      'Huy hiệu VIP nổi bật',
      'Voice Chat trong Watch Party',
 ...
 */
export const isSubscriptionPlanType = (value: unknown): value is SubscriptionPlanType => {
  return typeof value === 'number' && Object.values(SubscriptionPlanType).includes(value);
};

export const isSubscriptionStatus = (value: unknown): value is SubscriptionStatus => {
  return typeof value === 'number' && Object.values(SubscriptionStatus).includes(value);
};

export const isTeamMemberRole = (value: unknown): value is TeamMemberRole => {
  return typeof value === 'number' && Object.values(TeamMemberRole).includes(value);
};

export const isAuthProvider = (value: unknown): value is AuthProvider => {
  return typeof value === 'string' && Object.values(AuthProvider).includes(value as AuthProvider);
};

export const isSystemRole = (value: unknown): value is SystemRole => {
  return typeof value === 'string' && Object.values(SystemRole).includes(value as SystemRole);
};

export const isVerificationType = (value: unknown): value is VerificationType => {
  return typeof value === 'string' && Object.values(VerificationType).includes(value as VerificationType);
};

export const isUserAccountStatus = (value: unknown): value is UserAccountStatus => {
  return typeof value === 'string' && Object.values(UserAccountStatus).includes(value as UserAccountStatus);
};
 ...
 */
export const getSubscriptionFeatures = (planType: SubscriptionPlanType) => {
 ...
 */
export const buildPermission = (category: PermissionCategory, action: PermissionAction, qualifier?: string): string => {
...
export const parsePermission = (permission: string): {
  ...
  const [categoryAction, qualifier] = permission.split('_', 2);
  const [category, action] = categoryAction.split(':', 2);
