...
import { ContentType } from "./catalog-enum.ts";
...
export const REVENUE_SHARING = {
  DEFAULT_PLATFORM_PERCENTAGE: 25.0, // 25% platform fee
  DEFAULT_CREATOR_PERCENTAGE: 75.0, // 75% creator revenue
...
export const TAX_RATES = {
  VN: 0.1, // 10% VAT for Vietnam
  US: 0.0, // No tax for US (for now)
} as const;
...
export const isPaymentGateway = (value: unknown): value is PaymentGateway => {
  return (
    typeof value === "string" &&
    Object.values(PaymentGateway).includes(value as PaymentGateway)
  );
};
...
export const isRentalStatus = (value: unknown): value is RentalStatus => {
  return (
    typeof value === "string" &&
    Object.values(RentalStatus).includes(value as RentalStatus)
  );
};
...
export const calculateDiscountAmount = (
  ...
  if (discountType === DiscountType.PERCENTAGE) {
    const discount =
      Math.round(((originalAmount * discountValue) / 100) * 100) / 100;
 ...
 */
export const isCompletelyExpired = (
  gracePeriodEnd: Date,
  now: Date = new Date(),
): boolean => {
